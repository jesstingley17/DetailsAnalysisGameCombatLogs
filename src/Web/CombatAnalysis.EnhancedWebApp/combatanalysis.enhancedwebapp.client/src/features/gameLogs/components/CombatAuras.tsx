...
const CombatAuras = () => {
    ...
    useEffect(() => {
        ...
        const combatId: number = parseInt(searchParams.get("combat") || "0");
        const combatLogId: number = parseInt(searchParams.get("combatLog") || "0");
    ...
    useEffect(() => {
        ...
        const getCombat = async () => {
            try {
                ...
                console.error("Failed to fetch combat:", e);
            }
        }
    ...
    useEffect(() => {
        ...
        const getCombatAuras = async () => {
            try {
                const combatAuaras = await getCombatAurasByCombatId(combat?.id).unwrap();
                setCombatAuras(combatAuaras);
                setAllCombatAuras(combatAuaras);
            } catch (e) {
                console.error("Failed to fetch combat auras:", e);
            }
    ...
    const initSelectedCreatorCombatAuras = (creator: string) => {
        const auras = Array<CombatAuraModel>();
    ...
    const removeQuotes = (data: string) => {
        const newStr = data.slice(1, -1);
