...
const AddTagsToPost: React.FC<AddTagsToPostProps> = ({ postTags, setPostTags, t }) => {
    ...
    const addTag = () => {
        if (!tagInput.current || tagInput.current.value.length === 0 || postTags.length >= maxTags) {
            return;
        }
    ...
    const addTagByKeyHandle = (code: string) => {
        if (code !== "Enter" || !tagInput.current || tagInput.current.value.length === 0 || postTags.length >= maxTags) {
            return;
        }
    ...
    return (
        <div className="tags">
            <div className="tags__add">
                ...
                {showInputTag &&
                    <>
                        <input type="text" onBlur={addTag} onKeyDown={(e) => addTagByKeyHandle(e.code)} placeholder="Enter tag" ref={tagInput} />
                        <FontAwesomeIcon
                            icon={faBan}
