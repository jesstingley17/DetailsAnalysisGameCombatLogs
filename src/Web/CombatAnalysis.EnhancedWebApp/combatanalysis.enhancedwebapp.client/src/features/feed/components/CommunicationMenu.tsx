...
const CommunicationMenu: React.FC<CommunicationMenuProps> = ({ currentMenuItem, hasSubMenu }) => {
    ...
    const renderShortMenuItem = (menuItem: MenuItemModel) => (
    ...
    const renderMenuItem = (menuItem: MenuItemModel) => (
    ...
    const renderSubMenuByType = (subMenuItems: MenuItemModel[]) => {
        return menu === 0 ? renderSubMenu(subMenuItems) : renderShortSubMenu(subMenuItems);
    }

    const renderShortSubMenu = (subMenuItems: MenuItemModel[]) => (
        subMenuItems.map((subItem: MenuItemModel) => (
    ...
    const renderSubMenu = (subMenuItems: MenuItemModel[]) => (
        subMenuItems.map((subItem: MenuItemModel) => (
