...
import React, { memo, useEffect, useState, type SetStateAction } from 'react';
import { useNavigate } from 'react-router-dom';
import VoiceChatAudioDeviceSettings from './VoiceChatAudioDeviceSettings';

interface VoiceChatToolsBarProps {
	t: (key: string) => string;
	...
	setTurnOnCamera: (value: SetStateAction<boolean>) => void;
	turnOnMicrophone: boolean;
	setTurnOnMicrophone: (value: SetStateAction<boolean>) => void;
	audioOutputDeviceId: string;
	setAudioOutputDeviceId: (value: SetStateAction<string>) => void;
}

const VoiceChatToolsBar: React.FC<VoiceChatToolsBarProps> = ({
	t,
	properties,
	methods,
	screenSharing,
				...
				<div className="experimental-functions__content">
					<div className={`device ${turnOnCamera ? 'disabled' : ''}`}>
						...
						<FontAwesomeIcon
							...
							onClick={turnOnCamera ? () => {} : handleSwicthScreenSharing}
							onMouseEnter={turnOnCamera ? () => setShowWarning(true) : () => {}}
							onMouseLeave={turnOnCamera ? () => setShowWarning(false) : () => {}}
						/>
					</div>
					<div className={`device ${cameraExecute || screenSharing ? 'disabled' : ''}`}>
						...
						<FontAwesomeIcon
							...
							onClick={cameraExecute || screenSharing ? () => { } : () => setTurnOnCamera(!turnOnCamera)}
							onMouseEnter={screenSharing ? () => setShowWarning(true) : () => { }}
							onMouseLeave={screenSharing ? () => setShowWarning(false) : () => { }}
						/>
					</div>
				</div>
			</div>
