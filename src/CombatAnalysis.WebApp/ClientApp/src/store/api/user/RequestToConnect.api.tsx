...
export const RequestToConnectApi = UserApi.injectEndpoints({
    endpoints: builder => ({
        createRequestAsync: builder.mutation<RequestToConnect, RequestToConnect>({
            query: request => ({
                body: request,
        ...
        searchByOwnerId: builder.query<RequestToConnect, string>({
            query: (userId) => `/RequestToConnect/searchByOwnerId/${userId}`,
            providesTags: (result, error, id) => [{ type: 'RequestToConnect', id }]
        }),
        searchByToUserId: builder.query<RequestToConnect, string>({
